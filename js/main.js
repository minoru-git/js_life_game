...
(function() {   // グローバル変数を作りたくないので全体を関数にしてスコープ生成する & 即時実行したいので無名関数にする
    var SCREEN_SIZE = 500;  // キャンバスの幅(px)
    var SIDE_CELLS = 200;   // 一辺のセルの数
    var CELL_SIZE = SCREEN_SIZE / SIDE_CELLS;   // セルの幅
    var FPS = 10;   // フレームレート(一秒間の世代更新回数)
        ...
        for (var i=0; i<tempField.length; i++) {
            ...
            for (var s=-1; s<2; s++) {
                for (var t=-1; t<2; t++) {
                    ...
                    if (c>=0 && c<tempField.length) {   // 配列からはみ出していないか（上下の壁判定を兼ねて）
